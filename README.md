...
>
> 16:9 横版 | ESentient IP | 纯白手绘 | 少量红橙蓝中文批注 | Codex Skill
...
默认视觉 IP 是“ESentient”：一个戴黑色粗框眼镜的卷发小男孩，大耳朵、腮红雀斑、白 T 恤、表情平静。ESentient 不是吉祥物，不是贴纸，也不是站在角落里的装饰物，而是正在认真参与系统运转的荒诞工作者。
...
- 每张图的主题、核心意思、结构类型、ESentient 动作和中文标注建议
...
这个 skill 默认使用 Ian 的“ESentient 怪诞正文配图”风格：
...
```text
Use $ian-xiaohei-illustrations 为这篇中文文章设计并生成 5 张 ESentient 怪诞正文配图。
```
...
每张图写清楚：放在哪段后、主题、核心意思、结构类型、ESentient 在做什么、建议中文标注词。
...
```text
Use $ian-xiaohei-illustrations 把下面这篇文章生成 4 张 ESentient 怪诞正文配图。
...
6. 让 ESentient 承担核心动作
7. 每张图单独调用图像模型生成
8. 按 QA checklist 检查：白底、留白、ESentient 动作和形象特征、中文标注、非 PPT 感、非旧案例复刻
9. 保存最终 PNG，并报告用途和路径
...
- ESentient 必须承担核心动作；如果去掉 ESentient 画面仍然完全成立，说明他太装饰了。
- ESentient 的标志特征（卷发、黑框眼镜、大耳朵、白 T 恤）必须完整，否则视为形象漂移。
...
这套 ESentient 配图 Skill，只是我用 AI 搭建个人生产系统里的一个小工具。
